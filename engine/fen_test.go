...
func TestFENBuilder1(t *testing.T) {
	pos, _ := engine.FromFEN(engine.StartingPositionFEN)
	fen := engine.FEN(pos)
	pos2, _ := engine.FromFEN(fen)
...
func TestFENBuilder2(t *testing.T) {
	...
	pos, _ := engine.FromFEN(fenToTest)
	fen := engine.FEN(pos)
	pos2, _ := engine.FromFEN(fen)
	fen2 := engine.FEN(pos2)
...
func TestFromFEN1(t *testing.T) {
	pos, _ := engine.FromFEN("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1")
...
func TestFromFEN2(t *testing.T) {
	pos, _ := engine.FromFEN("rnbqkbnr/pp3ppp/8/2pPp3/P1P3N1/1P6/3PKPPP/RNB2B1R b Kq b4 0 6")
